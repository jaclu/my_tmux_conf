...
class BaseConfig(TmuxConfig):
    ...
    def floats_and_popups(self, key_floating_pane="", key_lazygit="", key_yazi=""):
        ...
        max_vers_popup_style = "3.8y"  # next "3.8z" doesn't support this
        ...
        if (
            used_any_popups
            and self.vers_ok(min_vers_popup_style)
            and not self.vers_ok(max_vers_popup_style)
        ):
    ...
    def handle_hooks(self):
        ...
        if self.vers_ok(2.4) and not self.is_tmate():
            ...
            w(  # needed for 2.5 to avoid label on new ses & win
                ...
                set-hook -g after-new-window{idx} "{borders_disable} {cmd_2}"
        ...
        if self.vers_ok(v_min_smart_hooks):
            ...
            # hooks exist in 2.3, but this check requires 2.4
            if self.vers_ok(2.4) and not self.is_tmate():
                idx = self.get_next_hook_array_idx()
            ...
            # w(
            #     """
            #         # tmux < 3.2 (and tmate) does not support nested if-shell with logical
            #         # operators to check zoom state and pane count.
...
if-shell -F '#{||:#{==:#{window_panes},1},#{!=:#{window_zoomed_flag},#{@zoom-state}}}' \\
{
    ...
    def auc_kill_tmux_server(self):  # used by iSH Console
        ...
        s += "Kill this server "
        if self.vers_ok(3.3):
            # Highlight conf-file and hostname, to make it absolutely clear
            # what tmux server will be killed
