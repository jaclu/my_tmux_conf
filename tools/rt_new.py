...
runs myt on a remote node, unlimittedjumps hosts can be used: rt jumphost/jump2/dest_node
...
class Host:
    ...
    def __init__(
        ...
        if use_quick:
            ...
            myt_cmd += " -q"
        if use_asdf_version:
            self.use_asdf_version = use_asdf_version
            myt_cmd += f" -a {use_asdf_version}"
        if use_tmate:
            self.use_tmate = True
            myt_cmd += " -m"
        if not server:
    ...
    def bounce_to(self, bounce_host):
        ...
        if self.use_2nd:
            rt_cmd += " -2"
        if self.use_asdf_version:
            rt_cmd += f" -a {self.use_asdf_version}"
        if self.use_tmate:
            rt_cmd += " -m"
        if self.use_mosh:
    ...
    def show_info(self):
        ...
        print(f"mosh:       {self.mosh_client} - {self.mosh_server} {self.mosh_ports}")
...
def parse_cmd_line():
    ...
    parser.add_argument(
        ...
        "--use_2nd",
        action="store_true",
        help="use 2nd tmux env on remote\n",
    )
    parser.add_argument(
        "-q",
        "--quick",
        action="store_true",
        help="quik connect, don't re-generate tmux.conf on remote\n",
    )
    parser.add_argument(
        "-a",
        "--asdf_version",
        help="set asdf tmux version on remote",
        action="store",
    )
    parser.add_argument(
        "-t",
...
def main():
    ...
    hc = get_host_class(
        ...
        asdf_version=args.asdf_version,
        use_tmate=args.use_tmate,
    ...
    if args.display:
        ...
        if hc.use_2nd or hc.use_quick or hc.use_asdf_version or hc.use_tmate:
            msg += " - myt Options:"
            if hc.use_2nd:
                msg += " -2"
            if hc.use_quick:
                msg += " -q"
            if hc.use_asdf_version:
                msg += f" -a {hc.use_asdf_version}"
            if hc.use_tmate:
                msg += " -m"
        print(msg + " ...")
        h_name = os.getenv("HOSTNAME_SHORT") or ""
        cmd = hc.full_command_line()
        # print(f"><> will run: {cmd}")
        proc = subprocess.run(cmd, shell=True, check=False)
        ex_code = proc.returncode
        # print(f"><> exit code: [{ex_code}]")
        if ex_code == 255:
            # remote tmux died, tends to leave the terminal messed up
            # do a reset
            msg = f"[{h_name}] rt: screen reset, since remote end"
            msg += f" died unexpectedly with exit code: {ex_code}"
            cmd = f'reset ; echo "{msg}"'
            subprocess.run(
                cmd,
                shell=True,
                check=False,
